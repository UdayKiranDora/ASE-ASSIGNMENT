...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, data, callback) {
        db.collection('tut10').insertOne( data, function(err, result) {
            if(err)           {
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('tut10').updateOne({"_id":ObjectId(id)},{$set:item}, function(err, result) {
            if(err)            {
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('tut10').deleteOne({"_id": ObjectId(id)}, function(err, result) {
